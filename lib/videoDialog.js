/******/ (function (modules) {
		...
		/***/ function (module, exports, __webpack_require__) {
			...
			AFRAME.registerComponent('video-popup', {
				...
				generateOpenIcon: function generateOpenIcon() {
					...
					function openPlane() {
						...
						if (AFRAME.utils.device.isMobile()) {
							...
							// var itembody = document.createElement('p');
							// var itemImg = document.createElement('img');
							var centeritemImg = document.createElement('video');//sandra
							var centerSide = document.createElement('div');//sandra

							// itemtitle.innerHTML = value;
							// itembody.innerHTML = body.replace(/\\n/g, '<br />');
							//itemImg.classList.add('itemImg');
							centeritemImg.classList.add('centeritemImg');
							centerSide.classList.add('centerPopup')//sandra
							// leftSide.classList.add('leftPopup');
							// rightSide.classList.add('rightPopup');
							// itemtitle.classList.add('itemTitle');
							// itembody.classList.add('itemBody');
							...
							centeritemImg.src = video;
							centeritemImg.controls = true;
							centeritemImg.autoloop = true;

							elContainer.classList.add('imgPopup');
							centerSide.appendChild(centeritemImg);
							// leftSide.appendChild(itemImg);
							// rightSide.appendChild(itemtitle);
							// rightSide.appendChild(itembody);
							// if (addImage) {
							...
							// elContainer.appendChild(leftSide);
							// elContainer.appendChild(rightSide);
							elContainer.appendChild(centerSide);
							elContainer.onmousedown = function () {
								var imgPopup = document.querySelectorAll('.imgPopup');
								for (let x = 0; x < imgPopup.length; x++) {
									$(imgPopup[x].remove());
				...
				generateTitle: function generateTitle() {
					...
					//this is the close up window for the video element sandra
					// var elContainer = document.createElement('div');
					// var leftSide = document.createElement('div');
					// var rightSide = document.createElement('div');
					// var itemtitle = document.createElement('p');
					...
					// //itemImg.classList.add('itemImg');
					// centeritemImg.classList.add('centeritemImg');
					// centerSide.classList.add('centerPopup');//sandra added
					// leftSide.classList.add('leftPopup');
					// rightSide.classList.add('rightPopup');
					// itemtitle.classList.add('itemTitle');
					// itembody.classList.add('itemBody');
					...
					// // 	if (itembody.innerHTML == '') {
					// // 		rightSide.style.cssText =
					// // 			'width:80%;height:40%;background:white;border-radius: 20px 20px 20px 20px;display:flex;justify-content:center;flex-direction:column;';
					// // 	}
					// // } else {
					// // 	if (itembody.innerHTML == '') {
					// // 		rightSide.style.cssText =
					// // 			'width:40%;height:80%;background:white;border-radius: 20px 20px 20px 20px;display:flex;justify-content:center;flex-direction:column;';
					// // 	}
					// // }
					// if (window.innerHeight > window.innerWidth) {
					// 	if (itembody.innerHTML == '') {
					...
					// }
					// // itemImg.src = src;
					// centeritemImg.src =src;
					// centeritemImg.autoplay = true;
					// centeritemImg.controls = true;
				...
				generateBody: function generateBody() {
					var _this$data4 = this.data,
						...
						src = _this$data4.playIconImage,
						title = _this$data4.title,
						addImage = _this$data4.addImage;

					var body = this.bodyEl || document.createElement('a-video');
					body.setAttribute(
						'id',
	...
	// 					var myVideo = document.querySelector('#Simulation1')
	...
	// 		}
		// });
				...
				generatePlayIcon: function generatePlayIcon() {
					var _this$data5 = this.data,
						src = _this$data5.src,//sandra change img to src
						playIconImage = _this$data5.playIconImage,
					...
					var playicon = document.createElement('a-image');
					playicon.setAttribute('position', '-0.14 -1.14 0.001');
					playicon.setAttribute('rounded', {
						height: 2.3,
						width: 2.5,
						topLeftRadius: 0,
						bottomLeftRadius: 0,
					}); 
					playicon.setAttribute('material', {
						shader: 'flat',
						src:playIconImage,
					});

					playicon.addEventListener('mouseenter', function hoverAns() {
						$('.a-canvas.a-grab-cursor:hover').css('cursor', 'pointer');
					});
					playicon.addEventListener('mouseleave', function hoverAns() {
						$('.a-canvas.a-grab-cursor:hover').css('cursor', 'grab');
					});
				...
				generateImage: function generateImage() {
					var _this$data5 = this.data,
						src = _this$data5.src,//sandra change img to src
						video = _this$data5.video,
						//image =  _this$data5.image
						width = _this$data5.imageWidth,
						height = _this$data5.imageHeight,
						dialogBoxHeight = _this$data5.dialogBoxHeight,
						multiple = _this$data5.multiple,
					...
					//thi sis clicking the hwote part
					...
					//this is the video plane
					var image = this.imageEl || document.createElement('a-rounded');
					var idname = this.el.getAttribute('id');					
					var matches = idname.match(/(\d+)/);
					//this is the corners of the video sandra
					image.setAttribute('rounded', {
						radius: 0.01,
						topRightRadius: 0.01,
						bottomRightRadius: 0.01,
						height: 1,
					});
					if (multiple) {
					...
					//this is the sacle of the video+ black canvass
					//image.setAttribute('scale', '5 3 1'); sandra
					image.setAttribute('scale', '7 4.2 1');
					//this is the width of the black rounded
				...
				generateDialogPlane: function generateDialogPlane() {
					...
					if (!AFRAME.utils.device.isMobile()) {
						...
						//image.setAttribute('imageHeight', '10');--sandra
						plane.classList.add('planeChecker');

						if (image) {
							plane.appendChild(image[0]);
							//this is the whitebg--sandra
						...
						//plane.appendChild(this.generatePlayIcon());
						//plane.appendChild(this.generateTitle());
						//this the small white square
