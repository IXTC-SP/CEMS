//Display content page after 5 seconds
setTimeout(function () {
...
//Check assessment mode is toggled
...
// //Rotate camera depending on area
...
//Rotate camera depending on area
function checkCamera(location) {
	if (location == '#loc7' || location == '#loc5' || location == '#loc8' ) {
		document.getElementById('skybox').setAttribute('rotation', '-4.85 0 0');
	} else if (location == '#loc3' || location == '#loc4'  || location == '#loc9'  || location == '#loc11'|| location == '#loc10'  || location == '#loc12') {
		document.getElementById('skybox').setAttribute('rotation', '-4.85 90 0');
	} else if (location == '#loc6') {
			document.getElementById('skybox').setAttribute('rotation', '-5.98 0 0');
	} else {
		document.getElementById('skybox').setAttribute('rotation', '-4.85 270 0');
	}
}
...
AFRAME.registerComponent('hotspots', {
	init: function () {
		this.el.addEventListener('reloadspots', function (evt) {
			//get the entire current spot group and make it visible
			var currspotgroup = document.getElementById(evt.detail.currspots);
			currspotgroup.setAttribute('visible', 'false');

			//get the entire new spot group and make it visible
...
AFRAME.registerComponent('play-pause', {	
	...
	//playicon.setAttribute('src', '#video')
	...
	var videoControls = document.querySelector('#videoControls');
		...
		if (rotation.x == 0) {
			tooltip.setAttribute('position', '0 0.75 -0.001');
		} else {
